...
class SettingsActivity : BaseNavigationActivity() {
    ...
    // OpenRouter fields
    private lateinit var editOpenRouterKey: android.widget.EditText
    private lateinit var spinnerOpenRouterModel: android.widget.Spinner
    private lateinit var btnSaveOpenRouterSettings: TextView
    ...
    private fun setupUI() {
        ...
        btnSaveOpenRouterSettings = findViewById(R.id.btnSaveOpenRouterSettings)
        setupOpenRouterSection()
    ...
    private fun loadAllSettings() {
        ...
        val savedKey = com.blurr.voice.utilities.ApiKeyManager.getOpenRouterKey(this)
        if (savedKey.isNotBlank()) {
            editOpenRouterKey.setText(savedKey)
        }
        val savedModel = com.blurr.voice.utilities.ApiKeyManager.getSelectedModel(this)
        val modelIndex = com.blurr.voice.utilities.ApiKeyManager.FREE_MODELS.indexOf(savedModel)
        if (modelIndex >= 0) spinnerOpenRouterModel.setSelection(modelIndex)
    }

    private fun setupOpenRouterSection() {
        // Populate model spinner
        val adapter = android.widget.ArrayAdapter(
            this,
            android.R.layout.simple_spinner_item,
            com.blurr.voice.utilities.ApiKeyManager.FREE_MODELS
        )
        adapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item)
        spinnerOpenRouterModel.adapter = adapter

        // Set saved model
        val savedModel = com.blurr.voice.utilities.ApiKeyManager.getSelectedModel(this)
        val idx = com.blurr.voice.utilities.ApiKeyManager.FREE_MODELS.indexOf(savedModel)
        if (idx >= 0) spinnerOpenRouterModel.setSelection(idx)

        // Save button
        btnSaveOpenRouterSettings.setOnClickListener {
            val key = editOpenRouterKey.text.toString().trim()
            val model = spinnerOpenRouterModel.selectedItem as? String
                ?: com.blurr.voice.utilities.ApiKeyManager.DEFAULT_MODEL
            ...
            com.blurr.voice.utilities.ApiKeyManager.saveOpenRouterKey(this, key)
            com.blurr.voice.utilities.ApiKeyManager.saveSelectedModel(this, model)
