...
class MainActivity : BaseNavigationActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Firebase auth is optional — we only check the local profile stored in SharedPreferences.
        // This allows the app to run without a Firebase account.
        try { auth = Firebase.auth } catch (e: Exception) {
            android.util.Log.w("MainActivity", "Firebase auth unavailable: ${e.message}")
        }
    ...
    override fun onStart() {
        super.onStart()
        val localProfile = UserProfileManager(this)
    ...
    private fun requestLimitIncrease() {
        val userEmail = try { auth.currentUser?.email } catch (e: Exception) { null }
            ?: com.blurr.voice.utilities.UserProfileManager(this).getEmail()
