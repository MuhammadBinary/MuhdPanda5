...
class TTSManager private constructor(private val context: Context) : TextToSpeech.OnInitListener {
    ...
    private suspend fun speakWithNativeTtsAndAwait(text: String) {
        try {
            isNativeTtsInitialized.await()
            val deferred = CompletableDeferred<Unit>()
            val utteranceId = "blurr_${System.currentTimeMillis()}"
            nativeTts?.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
                override fun onStart(id: String?) {}
                override fun onDone(id: String?) { deferred.complete(Unit) }
                @Deprecated("Deprecated in Java")
                override fun onError(id: String?) { deferred.complete(Unit) }
                override fun onError(utteranceId: String?, errorCode: Int) { deferred.complete(Unit) }
            })
            nativeTts?.speak(text, android.speech.tts.TextToSpeech.QUEUE_FLUSH, null, utteranceId)
    ...
    private suspend fun speak(text: String) {
        // No Google TTS key — use Android built-in TTS directly
        if (com.blurr.voice.BuildConfig.GOOGLE_TTS_API_KEY.isBlank()) {
            speakWithNativeTtsAndAwait(text)
            return
        }
        try {
